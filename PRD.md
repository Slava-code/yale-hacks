...
- **Clinical Knowledge Graph:** Pre-processed from ~300 synthetic clinical PDFs. Entities include patients, conditions, medications, procedures, lab results, visits, providers, family history, and disease references (institutional knowledge base). Relationships are edges (PATIENT→HAS_CONDITION, VISIT→RESULTED_IN→LAB_RESULT, PATIENT→PRESCRIBED→MEDICATION, etc.). Each node stores its source PDF path and page number for citation linking. Graph is stored on the GX10's local storage and loaded into memory at startup.
